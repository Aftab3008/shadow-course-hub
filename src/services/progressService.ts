...
// Mock database service - replace with actual Supabase calls when connected
...
class ProgressService {
  ...
  async saveProgress(courseId: string, lectureId: string, currentTime: number): Promise<void> {
    ...
    const existingProgress = JSON.parse(localStorage.getItem(this.storageKey) || '{}');
  ...
  async getProgress(courseId: string, lectureId: string): Promise<number> {
    ...
    if (progress) {
      console.log(`Progress loaded: ${progress.currentTime}s for lecture ${lectureId}`);
